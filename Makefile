# This Makefile is for the Google::ProtocolBuffers extension to perl.
#
...
#     ABSTRACT => q[simple interface to Google Protocol Buffers]
#     AUTHOR => [q[, ACKNOWLEDGEMENS, COPYRIGHT, Igor Gariev <gariev@hotmail.com>, the CISRT Gadgets Foundation <csirtgadgets.org>]]
#     BUILD_REQUIRES => { ExtUtils::MakeMaker=>q[6.59] }
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[Google-ProtocolBuffers]
#     EXE_FILES => [q[bin/protoc-perl]]
#     LICENSE => q[perl]
#     MIN_PERL_VERSION => q[5.008008]
#     NAME => q[Google::ProtocolBuffers]
#     NO_META => q[1]
...
MAKE = make
FULLEXT = Google/ProtocolBuffers
BASEEXT = ProtocolBuffers
PARENT_NAME = Google
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Google/ProtocolBuffers.pm
...
PM_TO_BLIB = lib/.DS_Store \
	...
	lib/Google/ProtocolBuffers/CodecIV32.pm \
	blib/lib/Google/ProtocolBuffers/CodecIV32.pm \
	lib/Google/ProtocolBuffers/CodecIV64.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Google/ProtocolBuffers.pm $(INST_MAN3DIR)/Google::ProtocolBuffers.$(MAN3EXT) 
...
pure_all :: $(INST_SCRIPT)/protoc-perl
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/protoc-perl 

$(INST_SCRIPT)/protoc-perl : bin/protoc-perl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/protoc-perl
	$(CP) bin/protoc-perl $(INST_SCRIPT)/protoc-perl
	$(FIXIN) $(INST_SCRIPT)/protoc-perl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/protoc-perl
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>simple interface to Google Protocol Buffers</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>, ACKNOWLEDGEMENS, COPYRIGHT, Igor Gariev &lt;gariev@hotmail.com&gt;, the CISRT Gadgets Foundation &lt;csirtgadgets.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,008008,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Class::Accessor" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Config::" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Math::BigInt" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Parse::RecDescent" VERSION="1.94" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" VERSION="0.52" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  ...
	  lib/Google/ProtocolBuffers/CodecIV32.pm blib/lib/Google/ProtocolBuffers/CodecIV32.pm \
	  lib/Google/ProtocolBuffers/CodecIV64.pm blib/lib/Google/ProtocolBuffers/CodecIV64.pm \
	  lib/Google/ProtocolBuffers/Compiler.pm blib/lib/Google/ProtocolBuffers/Compiler.pm \
	  lib/Google/ProtocolBuffers/Constants.pm blib/lib/Google/ProtocolBuffers/Constants.pm 
